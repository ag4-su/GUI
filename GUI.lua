...
GUI.Locale			= Menu.AddOptionCombo({ "GUI"},	"Localization", {"English", "Russian"}, 0)
GUI.SelectedTheme	= Menu.AddOptionCombo({ "GUI"},	"Theme", {"Default", "Dark", "Mono"}, 0)
...
GUI.PreCacheItems	= Menu.AddOptionBool({ "GUI", "GUIDB" },	"PreCache items", false)
GUI.PreCacheHeroes	= Menu.AddOptionBool({ "GUI", "GUIDB" },	"PreCache heroes", false)
GUI.PreCacheAb		= Menu.AddOptionBool({ "GUI", "GUIDB" },	"PreCache abilities", false)
...
GUI.Items							= {}
GUI.Font							= {}
GUI.Font.Menu						= Renderer.LoadFont("Tahoma", 17, Enum.FontWeight.BOLD)
GUI.Font.Content					= Renderer.LoadFont("Tahoma", 18, Enum.FontWeight.BOLD)
GUI.Font.ContentBold				= Renderer.LoadFont("Tahoma", 17, Enum.FontWeight.BOLD)
GUI.Font.ContentSmallBold			= Renderer.LoadFont("Tahoma", 15, Enum.FontWeight.BOLD)
GUI.Font.UltraSmallBold				= Renderer.LoadFont("Tahoma", 11, Enum.FontWeight.BOLD)
GUI.Font.Header						= Renderer.LoadFont("Tahoma", 19, Enum.FontWeight.BOLD)
GUI.Font.Search						= Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.MEDIUM)
GUI.Font.Footer						= Renderer.LoadFont("Tahoma", 17, Enum.FontWeight.MEDIUM)
GUI.GameState						= -2
GUI.Config							= "GUI"
GUI.Version							= 180602
...
function GUI.CheckKeys()
	...
	special_chars[Enum.ButtonCode.MOUSE_LEFT] = "left"
	special_chars[Enum.ButtonCode.MOUSE_RIGHT] = "right"
...
function GUI.Draw(leftclick, rightclick)
	...
	for k,v in pairs(GUI.CategoryName) do
		if GUI.ActiveCategories[k] then
			...
			Wrap.DrawTextCentered(GUI.Font.Header, nextlocx + 21, starty + 20, v, false)
	...
	Renderer.SetDrawColor(hex2rgb("#fff"))
	Wrap.DrawTextCentered(GUI.Font.ContentSmallBold, startx + 192, starty + 20, GUI.TextVersion, false)
	...
	if totalpage > 1 then
		...
		Wrap.DrawTextCentered(GUI.Font.Footer, startx + 115, starty + 540, GUI.TextValues[GUI.SelectedLanguage]["page"] .. (c + 1) .. GUI.TextValues[GUI.SelectedLanguage]["of"] .. totalpage, false)
		Renderer.SetDrawColor(hex2rgb("#fff"))
		...
		if (c + 1) ~= 1 then		
			...
			Wrap.DrawTextCentered(GUI.Font.ContentSmallBold, startx + 20, starty + 542, "<", false)
		end
		...
		if (c + 1) ~= totalpage then
			...
			Wrap.DrawTextCentered(GUI.Font.ContentSmallBold, startx + 213, starty + 542, ">", false)
		end
	end
...
function DrawMenuBox(k, startx, starty, workbox_x, limit_y, leftclick, rightclick)
	...
	if k == GUI.SelectedMenu then
		...
		if (utf8.len(t) > 70) then
			...
			if GUI.ShowDesc then
				Wrap.DrawTextCentered(GUI.Font.ContentSmallBold, tx + math.floor(tsize / 2), ty + math.floor(tsize / 2), "<", false)
			else
				Wrap.DrawTextCentered(GUI.Font.Header, tx + math.floor(tsize / 2), ty + math.floor(tsize / 2) - 4, "...", false)
			end
		end
		...
		if not GUI.ShowDesc then 
			...
			if GUI.Items[k]["perfect_version"] ~= nil and GUI.Items[k]["perfect_version"] ~= 0 then
				if GUI.Items[k]["perfect_version"] > GUI.Version then
					...
					Wrap.DrawTextCentered(GUI.Font.Search, startx + workbox_x + math.floor((900 - workbox_x) / 2), starty + 280, GUI.TextValues[GUI.SelectedLanguage]["version_s"] .. GUI.Items[k]["perfect_version"] .. GUI.TextValues[GUI.SelectedLanguage]["version_e"], false)
...
function DrawButton(x, y, text, width, leftclick, callback)
	...
	Wrap.DrawTextCentered(GUI.Font.ContentBold, x + math.floor(width / 2), y + 18, text, false)
end
...
function DrawKeyBox(click, x, y, value)
	...
	Renderer.SetDrawColor(hex2rgb(GUI.Colors.Accent))
	Wrap.DrawTextCenteredY(GUI.Font.Content, x, y + 15, value["name"], false)
	...
	if not inpos then
		...
		Wrap.DrawTextCentered(GUI.Font.ContentBold, x + width + math.floor(key_size / 2), y + 15, drawtext, false)
	else
		Wrap.DrawTextCentered(GUI.Font.ContentBold, x + width + math.floor(key_size / 2), y + 15, drawtext, false)
	end
...
function DrawPagination(key, click, x, y, size_x, size_y, showed, count, totalshowed)
	if GUI.Items[key]["page"] > 0 then 
		...
		Wrap.DrawTextCentered(GUI.Font.ContentSmallBold, x + math.floor(size_x / 2), y + math.floor(size_y / 2), "<", false)
	end
	...
	if totalshowed ~= count then 
		...
		Wrap.DrawTextCentered(GUI.Font.ContentSmallBold, x + size_x - 1 + math.floor(size_x / 2), y + math.floor(size_y / 2), ">", false)
	end
end
...
function DrawImageBox(click, x, y, value)
	...
	if value["search"] ~= nil and value["last_time"] ~= nil and value["last_time"] + 3 > os.clock() then
		...
		Renderer.SetDrawColor(255, 255, 255, 255)
		Wrap.DrawTextCentered(GUI.Font.Search, safe_x + math.floor(realWidth / 2), safe_y + 25 + math.floor(((y + value["size_y"]) - safe_y) / 2), value["search"], false)
	end
...
function DrawOrderBox(inpos, leftclick, rightclick, x, y, value)
	...
	for _, k in ipairs(sortedKeys) do
		...
		if hasValue(datawork, k) then
			...
			Wrap.DrawTextCentered(GUI.Font.Content, math.floor(tempx + (value["size_x"] / 2)), math.floor(y + 25 + (value["size_y"] / 2)), drawindex, false)
		end
...
function DrawSelectBox(leftclick, x, y, value)
	...
	if (GUI.Data[key] ~= CurentKey) then
		GUI.Set(key, datawork)
	end
...
function DrawSlider(leftclick, x, y, value)
	...
	Wrap.DrawTextCenteredY(GUI.Font.ContentSmallBold, x, y + 50, value["min"], false)
	...
	Wrap.DrawTextCenteredY(GUI.Font.ContentSmallBold, (x + 300) - math.floor(string.len(value["min"]) * 8), y + 50, value["max"], false)
	...
	if value["newvalue"] ~= nil then
		Wrap.DrawTextCenteredX(GUI.Font.ContentBold, x + (300 / 2), y + 45, value["newvalue"], false)
	else
		Wrap.DrawTextCenteredX(GUI.Font.ContentBold, x + (300 / 2), y + 45, GUI.Get(key), false)
	end
end
...
function GUI.GetThemeName()
	if Menu.GetValue(GUI.SelectedTheme) == 0 then return "Default" end
	if Menu.GetValue(GUI.SelectedTheme) == 1 then return "Dark" end
	if Menu.GetValue(GUI.SelectedTheme) == 2 then return "Mono" end
